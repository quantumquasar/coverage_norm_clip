...
loadData <- function(data_table, replicons){
  ...
  end_loaddata <- Sys.time()
  elapsed <- end_loaddata - start_loaddata
  print(elapsed)



  #####new, hopefully optimised, piece of code####
  wigs <- plyr::llply(data_table$file, function(x){
    this_wig <- fread(x,header = F, fill = T)
    this_wig$id <- rownames(this_wig)
    #print(head(this_wig))

    start_locs <- as.numeric(this_wig[V1=="variableStep", id])
    chr_names <- this_wig[V1=="variableStep", V2]
    ...
    for(y in 1:length(start_locs)){
      ...
      if(!(y==length(start_locs))){
        ...
        rl3 <- IRanges(as.numeric(this_wig[(start_locs[y]+1):(start_locs[y+1]-1), V1]),
                       as.numeric(this_wig[(start_locs[y]+1):(start_locs[y+1]-1), V1]))
        scores <- as.numeric(this_wig[(start_locs[y]+1):(start_locs[y+1]-1), V2])
        ...
        rl3 <- IRanges(as.numeric(this_wig[(start_locs[y]+1):length(this_wig$id), V1]),
                       as.numeric(this_wig[(start_locs[y]+1):length(this_wig$id), V1]))
        scores <- as.numeric(this_wig[(start_locs[y]+1):length(this_wig$id), V2])
        cov <- abs(coverage(rl3, weight = scores))
        if(isEmpty(list_rle)){
          list_rle <- RleList(cov)
        }
